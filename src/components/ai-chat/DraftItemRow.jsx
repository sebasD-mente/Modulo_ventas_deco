...
export default function DraftItemRow({
  ...
  return (
    <div className="flex items-center justify-between gap-2.5 p-2.5 rounded-xl bg-black border border-neutral-800 text-xs">
      ...
      <div className="flex-1 min-w-0">
        <span className="font-bold text-white block truncate">{item.baseTitle || item.description}</span>
        <span className="text-[9px] text-neutral-400 uppercase font-semibold block">{item.category || 'ARTE'}</span>
        <div className="flex items-center gap-1.5 mt-1">
          ...
          {itemSizes.length <= 1 ? (
            <span className="bg-[#222222] border border-neutral-700/80 rounded px-2 py-0.5 text-[10px] text-emerald-400 font-bold">
              {currentSize?.nombre || 'Portada de Álbum'} (Q{currentSize?.precio || item.unitPrice})
            </span>
          ) : (
            <select
              value={item.sizeId || currentSize?.sizeId}
              onChange={(e) => onUpdateSize?.(index, e.target.value)}
              className="bg-[#222222] border border-neutral-700 rounded-lg px-2 py-1 text-[11px] text-white font-bold focus:outline-none focus:border-white cursor-pointer min-h-[36px]"
            >
              {itemSizes.map((s) => (
        ...
        {onOpenSwap && (
          <button type="button" onClick={() => onOpenSwap(index)} className="text-[11px] text-neutral-400 hover:text-white flex items-center gap-1 underline transition-colors cursor-pointer mt-1 min-h-[32px] py-1">
            <RefreshCw className="w-3 h-3 text-emerald-400" /> Cambiar diseño
          </button>
        )}
      </div>
      <div className="flex flex-col items-end gap-1.5 shrink-0">
        ...
        <span className="font-black text-xs text-emerald-400">Q{(item.quantity * item.unitPrice).toFixed(2)}</span>
        <button
          type="button"
