...
import { Factory, Printer, Search, RefreshCw, Check, Layers } from 'lucide-react';
...
export default function ProductionManagementView() {
  ...
  const [activeTab, setActiveTab] = useState('queue'); // 'queue' | 'sheets'
  return (
    <div className="bg-[#121212] p-5 sm:p-7 rounded-[32px] sm:rounded-[36px] border border-neutral-800 shadow-2xl text-white max-w-2xl mx-auto space-y-6 select-none">
      ...
      {activeTab === 'sheets' ? <PrintSheetsSection isOp2Only={q.isOp2Only} isSuperAdmin={q.isSuperAdmin} /> : (
        <>
