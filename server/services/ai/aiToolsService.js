...
export const prepareSaleDraftDeclaration = { name: 'prepareSaleDraft', description: 'Prepara o actualiza un borrador formal de venta en el mostrador para confirmación del vendedor. Llamar ÚNICAMENTE en ventas o modificaciones.', parameters: { type: Type.OBJECT, properties: { items: { type: Type.ARRAY, items: { type: Type.OBJECT, properties: { productName: { type: Type.STRING }, quantity: { type: Type.INTEGER }, unitPrice: { type: Type.NUMBER }, size: { type: Type.STRING } }, required: ['productName', 'quantity'] } }, total: { type: Type.NUMBER }, paymentMethod: { type: Type.STRING, enum: ['EFECTIVO', 'TRANSFERENCIA', 'TARJETA'] }, notes: { type: Type.STRING }, customerName: { type: Type.STRING } }, required: ['items'] } };
export const searchCatalogDeclaration = { name: 'searchCatalog', description: 'Busca obras y pósters en el catálogo oficial de Deco Vintage por palabras clave, personaje, franquicia o artista.', parameters: { type: Type.OBJECT, properties: { query: { type: Type.STRING }, category: { type: Type.STRING } }, required: ['query'] } };
