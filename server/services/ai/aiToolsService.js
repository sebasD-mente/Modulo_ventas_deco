...
export const searchCatalogDeclaration = { name: 'searchCatalog', description: 'Busca obras y pósters en el catálogo oficial de Deco Vintage por palabras clave, personaje, franquicia o artista.', parameters: { type: Type.OBJECT, properties: { query: { type: Type.STRING, description: 'Término de búsqueda, personaje o franquicia a buscar.' }, category: { type: Type.STRING, description: 'Categoría opcional del catálogo (ej. CÓMICS, MUSICA, ANIME).' } }, required: ['query'] } };
...
export const getHourlySalesAnalyticsDeclaration = { name: 'getHourlySalesAnalytics', description: 'Obtiene el análisis de ventas por horario del evento en tiempo real: horas pico de venta, horarios de mayor recaudación y gráfico de distribución horaria.', parameters: { type: Type.OBJECT, properties: { eventId: { type: Type.STRING }, date: { type: Type.STRING } } } };
export const getTopSellingPostersDeclaration = { name: 'getTopSellingPosters', description: 'Retorna el podio oficial con los 3 pósters más vendidos del evento actual con fotos, unidades vendidas y recaudación contable real.', parameters: { type: Type.OBJECT, properties: { eventId: { type: Type.STRING }, limit: { type: Type.INTEGER } } } };
...
export const salesAssistantTools = [{ functionDeclarations: [prepareSaleDraftDeclaration, discardSaleDraftDeclaration, searchCatalogDeclaration, getEventKPIsDeclaration, getHourlySalesAnalyticsDeclaration, getTopSellingPostersDeclaration, getCashDrawerStatusDeclaration, getSellerShiftReportDeclaration, getProductionQueueStatusDeclaration, checkInventoryStockDeclaration] }];
...
export { getHourlySalesAnalytics as executeGetHourlySalesAnalytics, getTopSellingPosters as executeGetTopSellingPosters } from '../sales/saleAnalyticsService.js';
