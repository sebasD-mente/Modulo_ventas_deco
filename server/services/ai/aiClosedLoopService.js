...
import { constructDraftPayload, executeGetCashDrawerStatus, executeGetSellerShiftReport, executeGetProductionQueueStatus, executeCheckInventoryStock } from './aiToolsService.js';
...
export async function executeToolCall(call, { tenantId, eventId, date, message, resolved }) {
  ...
  try {
    if (call.name === 'prepareSaleDraft' && call.args) {
      ...
      return { event: { type: 'draft_sale', data: draft }, toolRecord: { name: call.name, args: call.args, result: draft } };
    }
    if (call.name === 'searchCatalog' && call.args?.query) {
      ...
      return { event: { type: 'suggested_posters', data: matches || [] }, toolRecord: { name: call.name, args: call.args, result: { matchesCount: matches?.length || 0, posters: (matches || []).slice(0, 6) } } };
    }
    if (call.name === 'getEventKPIs') {
      ...
      return { event: k ? { type: 'event_kpis', data: k } : null, toolRecord: { name: call.name, args: call.args, result: k } };
    }
    if (call.name === 'getCashDrawerStatus') {
      const s = await executeGetCashDrawerStatus(tenantId, effEvId);
      return { event: { type: 'cash_drawer_status', data: s }, toolRecord: { name: call.name, args: call.args, result: s } };
    }
    if (call.name === 'getSellerShiftReport') {
      const r = await executeGetSellerShiftReport(tenantId, effEvId, call.args?.sellerId || null);
      return { event: { type: 'seller_shift_report', data: r }, toolRecord: { name: call.name, args: call.args, result: r } };
    }
    if (call.name === 'getProductionQueueStatus') {
      const q = await executeGetProductionQueueStatus(tenantId, effEvId);
      return { event: { type: 'production_queue_status', data: q }, toolRecord: { name: call.name, args: call.args, result: q } };
    }
    if (call.name === 'checkInventoryStock' && call.args?.query) {
      ...
      return { event: { type: 'inventory_stock', data: st }, toolRecord: { name: call.name, args: call.args, result: st } };
    }
  } catch (err) {
...
export async function* streamClosedLoopFollowUp({ executedTools, formattedContents, systemInstruction, client, trailingTextTokens = 0 }) {
  ...
  const modelParts = executedTools.map(t => ({ functionCall: { name: t.name, args: t.args || {} } }));
  const toolParts = executedTools.map(t => ({ functionResponse: { name: t.name, response: (typeof t.result === 'object' && t.result !== null) ? t.result : { result: t.result } } }));
  ...
  try {
    const followUpStream = streamWithModelFallback({
      buildContentsAndConfig: () => ({ contents: closedLoopContents, config: { systemInstruction, safetySettings: salesAssistantSafetySettings } }),
      models: getActivePool(),
      client,
    });
    for await (const chunk of followUpStream) {
      const isBreak = chunk.text?.includes('Respuesta finalizada');
